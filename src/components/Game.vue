  ...
  <v-app :dark="dark" :light="!dark">
    <v-toolbar>
      ...
      <v-toolbar-items class="hidden-sm-and-down">
        ...
        <v-btn flat @click="switchDark"
          ><v-icon>invert_colors</v-icon
          >{{ dark ? $t("message.dark") : $t("message.light") }}</v-btn
        >
        ...
        <v-btn
          ...
          v-if="auth.$auth"
          ><v-icon :class="isSubscribed ? '' : 'animated infinite tada'"
            >notifications_none</v-icon
          >{{ isSubscribed ? "Unsubscribe" : "Subscribe" }}</v-btn
        >
        <v-btn flat @click="logout" v-if="auth.$auth"
          ><v-icon>exit_to_app</v-icon>{{ $t("message.logout") }}</v-btn
        >
      </v-toolbar-items>
    </v-toolbar>
    <v-navigation-drawer fixed app>
      ...
      <v-list dense class="pt-0">
        ...
        <v-list-tile to="/game/leaderboard">
          <v-list-tile-action> <v-icon>stars</v-icon> </v-list-tile-action>
          <v-list-tile-content>
            <v-list-tile-title>{{
...
messaging.usePublicVapidKey(
  "BL0N5Q6Ud09Gm8JLIuX_DEi1Iq2_ovXPX2WB_iZWSEZH1kK7GVa3h1S8-AfXfTpY3bQGrsrnhHHaukU98OUkzrY"
);
  ...
  methods: {
    ...
    async refreshToken() {
      if (this.$getTokenExp() && this.$getToken()) {
        try {
          const { data } = await this.$http.post("/auth/refreshToken", {
            token: this.$getToken()
          });
