<template>
  <div style="padding: 0 10px">
    ...
    <h1>
      <a :href="task.url" target="_newtab"
        >{{ isSolved ? "🔓" : "🔒" }} {{ task.name }}</a
      >
    </h1>
    ...
    <p>
      ...
      {{ task.added.replace("T", " ").replace("Z", "") }}
    </p>
    <p>
      {{ $t("message.madeby") }}:
      <router-link :to="'/game/user/' + task.by">{{ task.by }}</router-link>
    </p>
    {{ $t("message.solved") }}:
    <ul style="list-style: none">
      <li v-for="un in task.solvedBy.reverse().slice(0, 9)" :key="un">
        <router-link :to="'/game/user/' + un">🌟 {{ un }}</router-link>
      </li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      try {
        const { data } = await this.$http.post(
          "tasks/" + this.task.id + "/solved",
          { token: this.$getToken(), flag: this.flag }
        );
