  ...
  <div v-if="auth.$auth && auth.$isAdmin">
    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td>
          <a :href="props.item.url" target="_newtab">{{ props.item.url }}</a>
        </td>
...
export default {
  ...
  methods: {
    ...
    async save() {
      ...
      if (
        ...
        this.flagErrors.length ||
        this.urlErrors.length ||
        this.idErrors.length ||
        this.moneyErrors.length ||
        this.experienceErrors.length
      )
        return;
      ...
      if (this.editedIndex > -1) {
        try {
          const { data } = await this.$http.patch(
            "/tasks/" + this.editedItem.id,
            Object.assign({ token: this.$getToken() }, this.editedItem)
          );
          ...
          if (data.code === 200) {
            ...
            this.$swal(
              "Good job!",
              `${this.editedItem.name} successfully edited`,
              "success"
            );
          } else {
            console.error(data);
            ...
            this.$swal(
              "Oops, there is a problem!",
              `Failed to edit ${this.editedItem.name}`,
              "error"
            );
          }
        } catch (err) {
          ...
          if (data.code === 201) {
            ...
            setTimeout(
              () =>
                this.$swal(
                  "Good job!",
                  `${vm.editedItem.name} successfully added`,
                  "success"
                ),
              200
            );
            ...
            this.$swal(
              ...
              `Failed to add ${this.editedItem.name}`,
              "error"
            );
          }
        } catch (err) {
  ...
  computed: {
    ...
    flagErrors() {
      ...
      !this.$v.editedItem.flag.ctfFlag &&
        errors.push("Entered flag is not valid SafetyCTF flag");
      return errors;
    },
    urlErrors() {
      const errors = [];
      ...
      !this.$v.editedItem.url.url &&
        errors.push("Entered URL is not valid URL");
      return errors;
    },
    idErrors() {
      const errors = [];
    ...
    moneyErrors() {
      ...
      !this.$v.editedItem.money.required &&
        errors.push("Money reward is required");
      !this.$v.editedItem.money.between &&
        errors.push("Money reward isn't in 5-500$ range");
      return errors;
    },
    experienceErrors() {
      const errors = [];
      if (!this.$v.editedItem.experience.$dirty) return errors;
      !this.$v.editedItem.experience.required &&
        errors.push("Experience reward is required");
      !this.$v.editedItem.experience.between &&
        errors.push("Experience reward isn't in 5-500 points range");
      return errors;
    }
  }
};
